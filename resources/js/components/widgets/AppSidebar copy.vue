    ...
    <div class="overflow-y-auto">
        ...
        <div class="flex">
            <div class="flex-none w-12">
                <a class="">
                    <img class="w-10 h-10 rounded-full" :src="$assets_url + 'images/avatars/1.png'" alt="">
                </a>
                ...
                <template v-for="(item, m_index) in $store.state.system.menu" :key="m_index">
                    <a :class="($store.state.system.active_subs_1 == m_index) ? 'bg-white' : ''"
                        class="no-underline flex justify-between p-1 cursor-pointer text-base font-normal rounded-l-lg dark:text-white hover:bg-white dark:hover:bg-gray-700"
                        @click="toggleSideMenu(m_index, 'main')">
                        <div :class="($store.state.system.active_subs_1 == m_index) ? 'bg-indigo-100' : ''"
                            class="inline-block w-10 h-10 rounded-full align-middle  bg-indigo-100 text-center pt-1">
                    ...
                    <template v-for="(subitem, t_index) in item.menus" :key="t_index">
                        ...
                        <template v-if="subitem.list.length">
                            ...
                            <ul :id="'menu-' + m_index + '-' + t_index + '-list'"
                                ...
                                :class="(menu[m_index] && menu[m_index][t_index]) ? '' : 'hidden'"
                                class="py-1 bg-blue-200 border-b border-b-blue-300">
