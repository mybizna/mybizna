    ...
    <div class="mybizna-container accordion-container">
            ...
            <div class="mybizna-row mybizna-between-xs">
                <div class="mybizna-col">
                    <h2 class="content-header__title">
                        {{ window.$func.__("Opening Balances", "erp") }}
                    </h2>
                </div>
            </div>
        </div>
        ...
        <form action="" method="post" @submit.prevent="submitOBForm">
            <div class="mybizna-row">
                ...
                >
                    <label>{{ window.$func.__("Financial Year", "erp") }}</label>
            ...
            <div class="mybizna-row">
                <ul class="report-header" v-if="null !== fin_year">
                    <li>
                        ...
                        {{ window.$func.__("to", "erp") }} <em>{{ fin_year.end_date }}</em>
                    </li>
                </ul>
            </div>
            ...
            <!-- <div class="erp-accordion">
                <div class="erp-accordion-expand"
                    ...
                    <span class="wp-erp-ob-title">{{ window.$func.__('Accounts Receivable', 'erp') }}</span>
                </div>
                <table class="mybizna-table mybizna-form-table erp-accordion-expand-body" v-show="open5">
                    <thead>
                    <tr>
                        <th>{{ window.$func.__('People', 'erp') }}</th>
                        <th>{{ window.$func.__('Debit', 'erp') }}
                    ...
                    <tr class="add-new-line">
                        <td v-if="undefined === acct_rec" style="float: left;">
                            {{ window.$func.__( 'No People Found!', 'erp' ) }}
                        </td>
                        <td v-else colspan="9" style="text-align: left;">
                            <button @click.prevent="acct_rec.push({})" class="mybizna-btn btn--primary add-line-trigger"><i class="flaticon-add-plus-button"></i>{{ window.$func.__('Add People', 'erp') }}</button>
                        </td>
                    </tr>
                    </tbody>
                </table>
            ...
            <!-- <div v-if="acct_pay" class="erp-accordion">
                <div class="erp-accordion-expand"
                    ...
                    <span class="wp-erp-ob-title">{{ window.$func.__('Accounts Payable', 'erp') }}</span>
                </div>
                <table class="mybizna-table mybizna-form-table erp-accordion-expand-body" v-show="open6">
                    ...
                    <tr class="add-new-line">
                        ...
                        <td v-else colspan="9" style="text-align: left;">
                            <button @click.prevent="acct_pay.push({})" class="mybizna-btn btn--primary add-line-trigger"><i class="flaticon-add-plus-button"></i>{{ window.$func.__('Add People', 'erp') }}</button>
                        </td>
                    </tr>
                    </tbody>
                </table>
            ...
            <div v-if="tax_pay" class="erp-accordion">
                ...
                >
                    <thead>
                        <tr>
                            <th>{{ window.$func.__("Agency", "erp") }}</th>
                            <th>{{ window.$func.__("Debit", "erp") }}</th>
                            <th>{{ window.$func.__("Credit", "erp") }}</th>
                            <th></th>
                        </tr>
                    ...
                    <tbody>
                        ...
                        <tr class="add-new-line">
                            <td
                                ...
                                style="float: left"
                            >
                                {{ window.$func.__("No Bank Account Found!", "erp") }}
                            </td>
                            <!--   <td v-else colspan="9" style="text-align: left;">
                            <button @click.prevent="ledgers[7].push({})" class="mybizna-btn btn&#45;&#45;primary add-line-trigger"><i class="flaticon-add-plus-button"></i>{{ window.$func.__('Add Bank', 'erp') }}</button>
                        </td>-->
                        </tr>
                    </tbody>
                </table>
                ...
                <tbody>
                    <tr class="total-amount-row">
                        <td
                            ...
                            style="width: 60%"
                        >
                            <span>{{ window.$func.__("Total Amount", "erp") }}</span>
                        </td>
...
export default {
    ...
    computed: {
        ...
        finalTotalCredit() {
            ...
            if (
                this.accPayRec !== null &&
                this.accPayRec.bill_purchase_acc !== "0"
            ) {
    ...
    methods: {
        ...
        getOpbAccountDetailsPayableReceivable(startDate) {
            window.axios
                .get("/opening-balances/acc-payable-receivable", {
                    params: {
            ...
            for (const key in this.ledgers) {
                for (let idx = 0; idx < this.ledgers[key].length; idx++) {
                    if (
                        ...
                        this.debit_total += parseFloat(
                            this.ledgers[key][idx].debit
                        );
                    }
                    if (
                        Object.prototype.hasOwnProperty.call(
                        ...
                        this.credit_total += parseFloat(
                            this.ledgers[key][idx].credit
                        );
                    }
                }
            }
        ...
        validateForm() {
            ...
            this.acct_pay.forEach((element) => {
                if (
                    ...
                    this.form_errors.push(
                        window.$func.__("People is not selected in Accounts Payable.", "erp")
                    );
                }
            });
            ...
            this.tax_pay.forEach((element) => {
                if (
                    ...
                    this.form_errors.push(
                        window.$func.__("Agency is not selected in Tax Payable.", "erp")
                    );
                }
            });
        ...
        submitOBForm() {
            ...
            window.axios
                ...
                .then((res) => {
                    this.showAlert(
                        "success",
                        window.$func.__("Opening Balance Created!", "erp")
                    );
                })
                .catch((error) => {
                    throw error;
        ...
        getSelectedOB(year) {
            ...
            window.axios
                .get(`/opening-balances/${year.id}/count`)
                .then((response) => {
                ...
                .then(() => {
                    if (parseInt(count) === 0) {
                        ...
                        window.axios
                            .get(`/opening-balances/${year.id}`)
                            .then((response) => {
                                this.totalDebit = 0;
                                this.totalCredit = 0;
                                response.data.forEach((ledger) => {
                                    ledger.id = ledger.ledger_id;
                                    ledger.balance = this.transformBalance(
                                        ledger.balance
                                    );
                                    ...
                                    this.ledgers[ledger.chart_id].map(
                                        (item, i) => {
                                            if (
                                                item.ledger_id ==
                                                ledger.ledger_id
        ...
        fetchVirtualAccts(year) {
            window.axios
                .get(`/opening-balances/virtual-accts/${year.id}`)
                .then((response) => {
...
.accordion-container .erp-accordion {
    background: #fff;
    overflow: hidden;
    margin-bottom: 20px;
    box-shadow: 0 4px 18px -11px rgba(0, 0, 0, 0.25);
}
...
.accordion-container .erp-accordion table thead th :first-child {
    width: 40%;
}
.accordion-container .erp-accordion table thead th :not(:first-child) {
    width: 20%;
}

.accordion-container .erp-accordion table tbody td input {
    width: 100%;
}
...
.opening-fyear-select .multiselect {
    width: 200px;
}
</style>
