<template>
    <div class="mybizna-container purchase-create">
        <!-- Start .header-section -->
        <div class="content-header-section separator">
            <div class="mybizna-row mybizna-between-xs">
                <div class="mybizna-col">
                    <h2 v-if="orderToPurchase()">
                        {{ window.$func.__("Convert into Purchase", "erp") }}
                    </h2>
                ...
                <div class="mybizna-panel-body">
                    ...
                    <div class="mybizna-row">
                        ...
                        <div class="mybizna-col-sm-6">
                            <label>{{ window.$func.__("Reference No", "erp") }}</label>
                            <input
                                type="text"
                    ...
                    <table class="mybizna-table mybizna-form-table">
                        <thead>
                            <tr class="inline-edit-row">
                                ...
                                <th scope="col">{{ window.$func.__("Qty", "erp") }}</th>
                                <th scope="col">
                                    {{ window.$func.__("Unit Price", "erp") }}
                                </th>
                                <th scope="col">{{ window.$func.__("Amount", "erp") }}</th>
                                <th scope="col">{{ window.$func.__("VAT", "erp") }}</th>
                                <th scope="col"></th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            >
                                <th
                                    scope="row"
                                    class="col--products with-multiselect product-select"
                                >
                                    <multi-select
                                        v-model="line.product"
                                        :options="products"
                                        @input="setLineData(line)"
                                    />
                                </th>
                                <td class="col--qty">
                                    <input
...
export default {
    ...
    watch: {
        ...
        products() {
            if (this.$route.params.id) {
                this.transactionLines.map((item) => {
                    ...
                    item.tax_cat_id = product.length
                        ? product[0].tax_cat_id
                        : null;
                });
            }
        },
        taxRates() {
            if (this.$route.params.id) {
                let rate = this.taxZones.filter(
                    (item) => parseInt(item.id) === this.taxRate
                );
                this.taxRate = rate[0];
            }
        },
            ...
            this.transactionLines.map((item) => {
                if (item.product && item.qty && item.unitPrice) {
                    if (item.applyTax && item.tax_cat_id && rates.length) {
                        let taxRate = rates.filter(
                            (r) => r.sales_tax_category_id == item.tax_cat_id
                        );
    ...
    created() {
        if (this.$route.name === "PurchaseOrderCreate") {
            this.page_title = window.$func.__("Purchase Order", "erp");
            this.purchase_order = 1;
        } else {
            this.page_title = window.$func.__("Purchase", "erp");
    ...
    methods: {
        ...
        updatePurchase(requestData) {
            window.axios
                ...
                .then((res) => {
                    let message = window.$func.__("Purchase Updated!", "erp");
...
<style>
.purchase-create .dropdown {
    width: 100%;
}

.purchase-create .col--product {
    min-width: 500px;
}
...
.purchase-create .col--qty {
    width: 120px;
}

.purchase-create .col--qty input {
    width: 100% !important;
}

.purchase-create .col--uni_price,
.purchase-create .col--amount {
    width: 200px;
}
