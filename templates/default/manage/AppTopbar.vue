                ...
                <div class="flex justify-start pt-1 pl-1 mr-2 space-x-4 cursor-pointer text-blue-500">
                    ...
                    <a @click="applist" class="w-15">
                        ...
                        <div v-if="visibles.length"
                            :class="$store.state.system.applist ? 'text-cyan-200' : 'text-blue-500'"
                ...
                <ul id="main-menu" class="flex items-left justify-left">
                    ...
                    <template v-for="(visible, t_index) in visibles" :key="t_index">
                        <li v-if="countObjectKeys(visible.list)" class="group relative  p-2">
                            <a
                                class="text-sm text-gray-500 text-nowrap font-semibold cursor-pointer hover:text-blue-800">
                                {{ visible.title }}
                                <i class="fas fa-chevron-down" style="font-size:10px;"></i>
                            </a>
                            <ul
                                class="absolute hidden group-hover:block bg-white border shadow-l-lg mt-2 py-2 w-44 z-10">
...
export default {
    components: {
        SearchForm: window.$filters.fetchComponent('templates/manage/SearchForm.vue'),
