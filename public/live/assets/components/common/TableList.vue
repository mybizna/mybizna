        ...
        <div class="row table-search">
          <div class="col-sm-12">
            ...
            <div class="form-group table-search-button">
              <b-link color="info" @click="fetchRecords" size="sm">
                <b-icon icon="times"></b-icon> Reset Form
              </b-link>
              <div class="field-wrap text-center">
                <b-button variant="primary" @click="fetchRecords" size="sm">
          ...
          <table class="table">
            <thead>
              <tr>
                <th
                  scope="col"
                  v-for="(table_field, index) in table.headers"
                  :key="index"
...
export default {
  ...
  mounted() {
    ...
    this.$root.$on("bv::dropdown::show", (bvEvent) => {
      that.table_style = { "padding-bottom": "700px" };
    });

    this.$root.$on("bv::dropdown::hide", (bvEvent) => {
      that.table_style = { "padding-bottom": "0px" };
    });
  },
  data() {
    return {
    ...
    presetSearchForm() {
      ...
      t.search_fields.forEach(function (search_field) {
        if (!search_field.hidden) {
          var search_field_obj = formInputProcessorHelper(search_field, t);
        ...
        if (
          ...
          if (
            !Object.prototype.hasOwnProperty.call(t.select_list, select_name)
          ) {
      ...
      t.items.forEach(function (item, item_index) {
        t.table_fields.forEach(function (field, field_index) {
          if (field.pre_timezone) {
            ...
            curr_this.setValueByPerPath(t, field.path, item_index, tmp_value);
          }
        });
      });
    },
